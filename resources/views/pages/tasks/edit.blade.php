...
<div class="py-6">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <x-ui.page-header :title="__('Edit Task')">
            <x-slot name="actions">
                <x-ui.button :href="route('tasks.show', $task)" variant="secondary" icon="<path fill-rule='evenodd' d='M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z' clip-rule='evenodd' />">
                    {{ __('Back to Task') }}
                </x-ui.button>
            </x-slot>
        </x-ui.page-header>
            ...
            <form action="{{ route('tasks.update', $task) }}" method="POST" class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <x-ui.form-group :label="__('Title')" for="title" :error="$errors->first('title')" required class="md:col-span-2">
                        <x-ui.text-input id="title" name="title" type="text" class="block w-full" :value="old('title', $task->title)" required autofocus />
                    </x-ui.form-group>

                    {{-- Description --}}
                    <x-ui.form-group :label="__('Description')" for="description" :error="$errors->first('description')" class="md:col-span-2">
                        <x-ui.textarea id="description" name="description" rows="5" class="block w-full">{{ old('description', $task->description) }}</x-ui.textarea>
                    </x-ui.form-group>
                    ...
                    <x-ui.form-group :label="__('Due Date')" for="due_date" :error="$errors->first('due_date')">
                        <x-ui.text-input id="due_date" name="due_date" type="date" class="block w-full" :value="old('due_date', $task->due_date ? $task->due_date->format('Y-m-d') : '')" />
                    </x-ui.form-group>
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <x-ui.button :href="route('tasks.show', $task)" variant="secondary">
                        {{ __('Cancel') }}
                    </x-ui.button>
