...
@section('title', 'Create Smart Tag')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Create Smart Tag</h1>
            <p class="text-gray-600 dark:text-gray-300 mt-1">
                Set up dynamic task collections based on custom criteria
            </p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
        <form action="{{ route('smart-tags.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div class="border-b border-gray-200 dark:border-gray-700 pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="flex space-x-2">
                            ...
                            <input type="text" name="color_hex" id="color_hex" value="{{ old('color', '#4f46e5') }}" 
                ...
                <div class="mt-4">
                    ...
                    <textarea name="description" id="description" rows="2" 
                        class="form-textarea w-full" placeholder="Brief description of this smart tag">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input type="checkbox" name="filter_by_due_date" id="filter_by_due_date" value="1" 
                                {{ old('filter_by_due_date') ? 'checked' : '' }}
                                class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                        </div>
                        <div class="ml-3">
                            <label for="filter_by_due_date" class="font-medium text-gray-700 dark:text-gray-300">Filter by Due Date</label>
                            
                            <div class="mt-2 ml-6 {{ old('filter_by_due_date') ? '' : 'hidden' }}" id="due_date_options">
                                <div class="space-y-4">
                                    <div>
                                        <select name="due_date_operator" id="due_date_operator" class="form-select w-full sm:w-auto">
                                            <option value="before" {{ old('due_date_operator') == 'before' ? 'selected' : '' }}>Due before</option>
                                            <option value="after" {{ old('due_date_operator') == 'after' ? 'selected' : '' }}>Due after</option>
                                            <option value="between" {{ old('due_date_operator') == 'between' ? 'selected' : '' }}>Due between</option>
                                            <option value="overdue" {{ old('due_date_operator') == 'overdue' ? 'selected' : '' }}>Overdue</option>
                                            <option value="today" {{ old('due_date_operator') == 'today' ? 'selected' : '' }}>Due today</option>
                                            <option value="next_7_days" {{ old('due_date_operator') == 'next_7_days' ? 'selected' : '' }}>Due in next 7 days</option>
                                        </select>
                                    </div>
                                    
                                    <div id="due_date_value_container" class="{{ in_array(old('due_date_operator'), ['overdue', 'today', 'next_7_days']) ? 'hidden' : '' }}">
                                        <div class="flex flex-wrap gap-4">
                                            <div>
                                                <input type="date" name="due_date_value" id="due_date_value" 
                                                    value="{{ old('due_date_value') }}" class="form-input">
                                            </div>
                                            
                                            <div id="due_date_end_container" class="{{ old('due_date_operator') == 'between' ? '' : 'hidden' }}">
                                                <span class="mx-2">and</span>
                                                <input type="date" name="due_date_end_value" id="due_date_end_value" 
                                                    value="{{ old('due_date_end_value') }}" class="form-input">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="flex items-center">
                                        <input type="checkbox" name="status_in_progress" id="status_in_progress" value="1" 
                                            {{ old('status_in_progress') ? 'checked' : '' }}
                                            class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                        ...
                        <div class="ml-3">
                            ...
                            <div class="mt-2 ml-6 {{ old('filter_by_tags') ? '' : 'hidden' }}" id="tags_options">
                                <div class="space-y-4">
                                    <div>
                                        ...
                                        <div class="flex items-center mb-2">
                                            <input type="radio" name="tags_filter_type" id="has_no_tags" value="has_none" 
                                                {{ old('tags_filter_type') === 'has_none' ? 'checked' : '' }}
                                                class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
                                        ...
                                        <div class="flex items-center">
                                            <input type="radio" name="tags_filter_type" id="has_specific_tags" value="has_specific" 
                                                {{ old('tags_filter_type') === 'has_specific' ? 'checked' : '' }}
                                                class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
                                    ...
                                    <div id="specific_tags_container" class="ml-6 {{ old('tags_filter_type') === 'has_specific' ? '' : 'hidden' }}">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="flex items-center">
                                        <input type="radio" name="has_time_entries" id="has_time_entries_no" value="0" 
                                            {{ old('has_time_entries') === '0' ? 'checked' : '' }}
                                            class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
